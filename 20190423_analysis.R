...
#  Rillig et al.(20XX) 
#
#  Appendix X: joint effect size estimation and visualization
#
#
#####################################################################
...
responses = c("Rich.ASV", "Comm.Disp", "Bacteria", "Fungi", "Total_microbes", "Ratio_fungi_to_bacteria", "WSA", "wdpt.sec.", "Percent.decomposition..corr.", "CO2_ppm_ThirdWeek")
...
# # Ploting Effect Size with the mean & its 95% confidence intervals
# g_ES = 
#   ggplot() +
#   ylab("") +
#   theme(axis.text.y=element_blank(), axis.ticks.y = element_blank()) +
#   xlab("Effect Size: difference in mean from control") + 
#   scale_y_discrete(limits = rev(treatment)) +
#   geom_estci(data=response_ES, aes(x = mean, y = target, xmin=X2.5., xmax=X97.5., 
#                                    xintercept=0), 
#              colour = response_ES$color_p,
#              size=0.7, ci.linesize = 0.9, position=position_nudge(y = -0.05))
...
g_ms = 
  ...
  xlim(low = min(unlist(ES_plot)*1.1), high=max(unlist(ES_plot))*1.1) +
  #coord_flip() +
